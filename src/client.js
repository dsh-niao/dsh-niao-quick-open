/**
 * dsh-niao-quick-open — browser half.
 *
 * Registers three session-header icon buttons (configure editor, open in
 * editor, open in file manager) plus a copy-folder-name button in the
 * `conversation.session.header.utilities` slot. All Host work happens over
 * the same-origin JSON route `/api/dsh-niao-quick-open`.
 *
 * Pure React.createElement — no JSX — so the browser bundle needs no JSX
 * transform, only import rewriting.
 *
...
/** Same-origin JSON POST to the Host route. */
...
const getEditor = () => { try { return window.localStorage.getItem(EDITOR_KEY) } catch { return null } }
const setEditor = (id) => { try { window.localStorage.setItem(EDITOR_KEY, id) } catch { /* ignore */ } }
...
function makeIcon(paths) {
  return h('svg', { width: 16, height: 16, viewBox: '0 0 16 16', fill: 'none', xmlns: 'http://www.w3.org/2000/svg' },
    paths.map((d) => h('path', { key: d.slice(0, 8), d, fill: 'currentColor' })))
}

const gearSvg = makeIcon([GEAR_PATH, GEAR_INNER])
const codeSvg = makeIcon([CODE_PATH])
const folderSvg = makeIcon([FOLDER_PATH])
const copySvg = makeIcon([COPY_PATH])
...
function HeaderActions(props) {
  ...
  useEffect(() => () => {
    if (copyTimer.current) { try { copyTimer.current() } catch { /* ignore */ } copyTimer.current = null }
  }, [])

  const openMenu = () => {
    setMenuOpen(true)
    setEditorDraft(getEditor() || '')
    if (editors === null) rpc('list-editors', {}).then((list) => { setEditors(Array.isArray(list) ? list : []) }).catch(() => { setEditors([]) })
  }
  const saveMenu = () => { setEditor(editorDraft || null); setMenuOpen(false) }
  const cancelMenu = () => { setMenuOpen(false) }
  const runFinder = () => { if (cwd) rpc('open-in-finder', { cwd }).catch(() => {}) }
  const runCopyName = async () => {
    if (!folderName) return
    let ok = false
    try {
      if (navigator.clipboard && navigator.clipboard.writeText) { await navigator.clipboard.writeText(folderName); ok = true }
    } catch { ok = false }
    if (!ok) {
      try {
    ...
    setCopied(true)
    if (copyTimer.current) { try { copyTimer.current() } catch { /* ignore */ } copyTimer.current = null }
    copyTimer.current = window.setTimeout(() => { setCopied(false); copyTimer.current = null }, 1000)
  }
  const runEditor = () => {
    if (!cwd) return
    const id = getEditor()
    if (!id) { openMenu(); return }
    rpc('open-with-editor', { cwd, editorId: id }).then((r) => {
      if (r && r.reason === 'not-found') openMenu()
    }).catch(() => {})
  }

  const iconBtn = (title, icon, onClick, disabled, tipText) => h('button', {
    type: 'button',
    className: 'dsh-hdr-btn',
    'aria-label': title,
    disabled: disabled || false,
    onClick: (e) => { e.stopPropagation(); onClick() },
  }, icon, h('span', { className: 'dsh-hdr-tip' }, tipText || title))

  const menuEl = menuOpen ? h('div', { className: 'dsh-hdr-menu', ref: menuRef, onClick: (e) => e.stopPropagation() },
    h('div', { className: 'dsh-hdr-menu-head' }, '设置'),
    h('div', { className: 'dsh-hdr-menu-field' },
      h('label', { className: 'dsh-hdr-menu-label' }, '常用编辑器'),
      editors === null ? h('div', { className: 'dsh-hdr-menu-note' }, '正在检索已安装的编辑器…')
        : editors.length === 0 ? h('div', { className: 'dsh-hdr-menu-note' }, '未检测到支持的编辑器')
        : h('select', {
          className: 'dsh-hdr-select',
          value: editorDraft,
          onChange: (e) => { e.stopPropagation(); setEditorDraft(e.target.value) },
        },
          h('option', { value: '' }, '未设置'),
          editors.map((ed) => h('option', { key: ed.id, value: ed.id }, ed.name))),
    ),
    h('div', { className: 'dsh-hdr-menu-foot' },
      h('button', { type: 'button', className: 'dsh-hdr-menu-btn', onClick: (e) => { e.stopPropagation(); cancelMenu() } }, '取消'),
      h('button', { type: 'button', className: 'dsh-hdr-menu-btn primary', onClick: (e) => { e.stopPropagation(); saveMenu() } }, '保存')),
  ) : null

  return h('div', { className: 'dsh-hdr-actions', onClick: (e) => e.stopPropagation() },
