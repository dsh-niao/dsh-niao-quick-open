...
async function detectPlatform(ctx) {
  if (await exists(ctx, '/Applications')) return 'macos'
  if (await exists(ctx, 'C:\\Windows')) return 'windows'
  return 'linux'
}
...
async function listEditors(ctx) {
  ...
  if (platform === 'macos') {
    for (const dir of ['/Applications', '/System/Applications']) {
      let entries = []
      try { entries = await ctx.fs.listDir(dir) } catch { entries = [] }      for (const entry of entries) {
...
/** 请求处理器：GET 返回编辑器列表，POST 按 action 分发。 */
async function handle(ctx, req, res) {
  if (req.method === 'GET') {
    responseJson(res, 200, { ok: true, value: { editors: await listEditors(ctx) } })
    return
  }
  if (req.method !== 'POST') {
