 ...
 * @property {boolean} enabled 「工作区快捷按钮」显示开关。
 * @property {string} editor 常用编辑器 id；空串 = 未设置。
 * @property {boolean} showRestart 左下角「硬性重启」按钮显示开关。
 * @property {boolean} menuQuickActions 工作区行菜单快捷按钮开关。
 * @property {boolean} sessionDoneMark 「会话待记标记」开关。
 * @property {boolean} flatListStyle 「单列表增强样式」开关。
 * @property {boolean} headerViewSwitches 「工作区栏头部增强」开关。
 * @property {boolean} conversationNav 「用户消息导航条」开关。
 * @property {string} configFile 设置面板持久化配置文件路径（高级项，通常保持默认）。
 */

/** 配置 schema：Cordis 加载时校验并填充默认值（最佳实践）。 */
export const Config = Schema.object({
  enabled: Schema.boolean().default(true),
  editor: Schema.string().default(''),
  showRestart: Schema.boolean().default(true),
  menuQuickActions: Schema.boolean().default(false),
  sessionDoneMark: Schema.boolean().default(false),
  flatListStyle: Schema.boolean().default(true),
  headerViewSwitches: Schema.boolean().default(false),
  conversationNav: Schema.boolean().default(true),
  configFile: Schema.string().default(join(homedir(), '.dsh', 'dsh-niao-quick-open.config.json')),
})
...
/** 面板可持久化的键（设置面板可修改、写入配置文件的字段）。 */
const PERSIST_KEYS = ['enabled', 'editor', 'showRestart', 'menuQuickActions', 'sessionDoneMark', 'flatListStyle', 'headerViewSwitches', 'conversationNav']
...
/**
 * 合并配置：patch config（Cordis schema 已填充默认值并校验）← 配置文件
 * （设置面板持久化的运行期修改，最高优先级）。configFile 路径来自
 * patch config（schema 默认 ~/.dsh/dsh-niao-quick-open.config.json）。
 */
  ...
  try {
    switch (action) {
      ...
      case 'set-config': {
        ...
        if (typeof patch.conversationNav === 'boolean') cfg.conversationNav = patch.conversationNav
...
function scheduleRestart() {
  // 重建启动参数时必须用 process.execArgv（Node 把 --import tsx/esm 这类
  // flag 剥进了 execArgv，不在 argv 里），并且把脚本入口转成绝对路径，
  // 用脚本目录作 cwd，否则子进程相对路径解析失败且 tsx 找不到。
  const entry = resolve(process.argv[1])
  const nextArgv = [...process.execArgv, entry, ...process.argv.slice(2)]
  const cwd = dirname(entry)
  const launch = { file: process.execPath, args: nextArgv, cwd }
  // 重启代理：等待 3080 端口真正释放后再拉起新实例，避免新实例因
  // 旧进程端口未释放而 EADDRINUSE 静默失败；失败时写日志留证。
  const proxy = `
...
const port = 3080;
const logErr = path.join(os.tmpdir(), 'dsh-niao-restart.err.log');
const note = (line) => { try { fs.appendFileSync(logErr, '[dsh-niao] ' + line + '\\n') } catch {} };
...
(async () => {
  ...
  await sleep(300);
  try {
    const child = spawn(file, args, { cwd, detached: true, stdio: 'ignore', env: process.env });
    child.unref();
    note('spawned replacement pid=' + child.pid);
  } catch (error) {
    note('could not spawn replacement: ' + (error && error.message ? error.message : error));
  }
})();
`
  try {
