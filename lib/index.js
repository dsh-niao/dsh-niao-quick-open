...
/**
 * 读取会话的「用户消息」列表（与前端界面 transcript 顺序一致，按 seq 升序）。
 *
 * 为什么遍历全部 append user 事件而不是 session.surface.nodes：
 * DSH 客户端界面（human transcript）故意不跟随 surface 的 replace 遮蔽
 * （被遮蔽的旧消息在界面上依然显示，这是产品设计），因此前端需要用
 * 「已删除」标记把对应行隐藏起来。这里返回全部 append user 消息
 * （含已被本插件删除的），并用 deleted 标志标注——顺序与界面一致，
 * 前端可一一配对。
 *
 * 排除：source.kind 非 user 的全部系统注入消息（tool / plugin /
 * agent-instructions / skill-catalog 等，前端只把 kind==='user' 渲染为
 * 用户消息行）、无文本消息。
 * @returns {Array<{ seq: number, role: 'user', text: string, deleted: boolean }>}
 */
function surfaceItemList(session) {
  const events = session.events
  // 收集被本插件删除占位（id 以 nio-delete: 开头）遮蔽的事件 seq。
  const shadowed = new Set()
  for (const ev of events) {
    if (!ev || ev.type !== 'assistant/message') continue
    const msg = ev.data && typeof ev.data === 'object' && ev.data.message ? ev.data.message : {}
    if (typeof msg.id !== 'string' || !msg.id.startsWith('nio-delete:')) continue
    if (Array.isArray(ev.sourceEventSeqs)) {
      for (const s of ev.sourceEventSeqs) {
        if (Number.isSafeInteger(s)) shadowed.add(s)
      }
    }
  }
  const list = []
  for (const ev of events) {
    if (!ev || ev.type !== 'user/message' || ev.surfaceOp !== 'append') continue
    // 严格只保留真实用户输入（source.kind === 'user'），与前端
    // data-chat-flow-kind="user" 的行一一对应；agent-instructions /
    // skill-catalog 等注入消息在前端渲染为 context 行，混入会导致配对错位。
    const src = ev.data && typeof ev.data === 'object' && ev.data.source ? ev.data.source : null
    if (!src || typeof src.kind !== 'string' || src.kind !== 'user') continue
    const text = userPromptText(ev.data)
    if (!text) continue
    list.push({ seq: ev.seq, role: 'user', text: text.slice(0, 200), deleted: shadowed.has(ev.seq) })
  }
  return list
}
