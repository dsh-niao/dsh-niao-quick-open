...
function applyConfigPatch(next) {
  ...
  if (next && typeof next.flatListStyle === "boolean" || typeof next.headerViewSwitches === "boolean") pluginConfig.flatListStyle = next.flatListStyle;
  if (changed) {
...
function ensureHeaderViewSwitches() {
  ...
  if (!actions.querySelector("[data-nio-hvswitch]")) {
    ...
    const mkBtn = (key, iconSvg, aria) => {
      ...
      btn.addEventListener("click", (e) => {
        ...
        pickMenuOption(targetItemText(kind, st2[kind]));
      });
      return btn;
    };
    const groupBtn = mkBtn("groupBy", groupBySvg, zh2 ? "\u5207\u6362\u5206\u7EC4\u65B9\u5F0F" : "Toggle grouping");
    const orderBtn = mkBtn("orderBy", orderBySvg, zh2 ? "\u5207\u6362\u6392\u5E8F\u65B9\u5F0F" : "Toggle sorting");
  ...
  if (groupTip) groupTip.textContent = zh ? `\u5F53\u524D\u4E3A${modeLabel("groupBy", st.groupBy)}\uFF0C\u70B9\u51FB\u540E\u5207\u6362\u4E3A${modeLabel("groupBy", st.groupBy === "flat" ? "workspace" : "flat")}` : `Now ${modeLabel("groupBy", st.groupBy)}, click to switch to ${modeLabel("groupBy", st.groupBy === "flat" ? "workspace" : "flat")}`;
  if (orderTip) orderTip.textContent = zh ? `\u5F53\u524D\u4E3A${modeLabel("orderBy", st.orderBy)}\uFF0C\u70B9\u51FB\u540E\u5207\u6362\u4E3A${modeLabel("orderBy", st.orderBy === "manual" ? "updated" : "manual")}` : `Now ${modeLabel("orderBy", st.orderBy)}, click to switch to ${modeLabel("orderBy", st.orderBy === "manual" ? "updated" : "manual")}`;
}
registerUIApply(() => {
  try {
